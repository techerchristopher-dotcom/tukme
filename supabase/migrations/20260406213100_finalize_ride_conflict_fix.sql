-- Hotfix: finalize_ride_financials used ON CONSTRAINT with a unique index name.
-- Use generic ON CONFLICT DO NOTHING (guarded by unique partial index on ride_id for ride_earning).
