...
import { getCompletedRides } from '@/lib/adminApi';
import { formatAriary } from '@/lib/money';
import type { CompletedRideRow } from '@/lib/types';

export default function RidesPage() {
  const { businessDate, setBusinessDate } = useBusinessDate();
  const [finalized, setFinalized] = useState<'all' | 'true' | 'false'>('all');
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [data, setData] = useState<{ items: CompletedRideRow[]; count: number; limit: number; offset: number } | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    setLoading(true);
    void (async () => {
      const res = await getCompletedRides({ date: businessDate, finalized, limit, offset });
      if (cancelled) return;
      if (res.error) {
    ...
    <RequireAuth>
      <AdminShell title={`Courses ${titleSuffix}`.trim()}>
          ...
          <div className="flex flex-wrap items-end gap-4">
            ...
            <label className="flex flex-col gap-1 text-sm">
              ...
              >
                ...
                <option value="true">Finalisées</option>
                <option value="false">Non finalisées</option>
              </select>
            </label>
          </div>
        ...
        {!loading && !error && items.length === 0 ? (
          <div className="mt-4 text-sm text-zinc-600">Aucune course.</div>
        ) : null}
                ...
                {items.map((r) => (
                  <tr key={r.ride_id} className="hover:bg-zinc-50">
                    <td className="border-b border-zinc-100 px-3 py-2">
                      <div className="font-mono text-xs">{r.ride_completed_at ?? '—'}</div>
                    </td>
                    <td className="border-b border-zinc-100 px-3 py-2">
                      <div className="font-medium">{r.driver_full_name ?? '—'}</div>
                      <div className="text-xs text-zinc-500">{r.driver_phone ?? ''}</div>
                    </td>
                    <td className="border-b border-zinc-100 px-3 py-2">{formatAriary(r.fare_total_ariary)}</td>
                    <td className="border-b border-zinc-100 px-3 py-2">
                      {formatAriary(r.platform_commission_ariary)}
                    </td>
                    <td className="border-b border-zinc-100 px-3 py-2">
                      {formatAriary(r.driver_gross_ariary)}
                    </td>
                    <td className="border-b border-zinc-100 px-3 py-2">
